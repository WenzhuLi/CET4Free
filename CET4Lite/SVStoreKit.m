...
//    _requestFrom = RequestFromProduct;
    if ([SKPaymentQueue canMakePayments]) {
        ...
        [self performSelector:@selector(timeout) withObject:nil afterDelay:100.0];
...
//    [self recordTransaction: transaction];
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(storeKitDidRestoredIdentifier:)]) {
        [_delegate storeKitDidRestoredIdentifier:transaction.payment.productIdentifier];
    }
    [[SKPaymentQueue defaultQueue] finishTransaction: transaction];
}
