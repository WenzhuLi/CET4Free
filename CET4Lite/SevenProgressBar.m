...
-(id)initWithFrame:(CGRect)frame andbackImg:(UIImage*)img frontimg:(UIImage *)fimg{
    ...
    if (self) {
        ...
        frontImg = [[UIImageView alloc]initWithFrame:CGRectMake(5, 0, 0, frame.size.height)];
        frontImg.image = fimg;
...
-(void)setProgress:(float)progress
{
	...
	frontImg.frame=CGRectMake(5, 0, progress*(self.frame.size.width - 5), self.frame.size.height);
}
//-(void)dealloc{
